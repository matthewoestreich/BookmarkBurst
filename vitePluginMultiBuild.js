 ...
 * from a single file.
 * @param {{ builds: vite.InlineConfig[] }} options
 * @returns
 */
export function multiBuild(options = { builds }) {
  return {
    name: "vite-plugin-multi-build",
    apply: "build",
    closeBundle: async () => {
      for (let i = 0; i < options.builds.length; i++) {
        ...
        await vite.build(vite.defineConfig(options.builds[i]));
      }
    },
  };
}
